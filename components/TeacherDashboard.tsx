...
interface Props {
  goals: LearningGoal[];
  users: User[];
  onAddGoal: (subject: Subject, title: string, description: string) => void;
...
export const TeacherDashboard: React.FC<Props> = ({ 
  ...
  const getUserName = (userId: string) => {
    return getUser(userId)?.name || 'Onbekende leerling';
  };

  // Helper om per doel de laatste reflectie per leerling te pakken
  const getLatestReflectionsPerStudent = (reflections: ReflectionEntry[]) => {
    const studentMap: Record<string, ReflectionEntry[]> = {};
    
    // Groepeer eerst alle reflecties per student
    reflections.forEach(r => {
      if (!studentMap[r.userId]) studentMap[r.userId] = [];
      studentMap[r.userId].push(r);
    });

    // Sorteer elke groep op tijd (nieuwste eerst)
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <h1 className="text-3xl font-bold text-slate-800">Leerkracht Dashboard</h1>
        ...
        <div className="animate-fadeIn">
          <div className="flex justify-between items-center mb-6">
            ...
            <button
              ...
              className="bg-blue-600 text-white font-bold py-2 px-6 rounded-xl shadow hover:bg-blue-700 transition-colors flex items-center gap-2"
            >
          ...
          <div className="grid grid-cols-1 gap-4">
            {goals.map((goal) => (
              <div key={goal.id} className="bg-white p-4 rounded-xl shadow-sm border border-slate-100 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-slate-500 text-sm">{goal.subject}</p>
                  </div>
                </div>
                <div className="flex items-center gap-4">
      ...
      {activeTab === 'REFLECTIONS' && (
        <div className="animate-fadeIn space-y-12">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-slate-500 text-sm italic">Klik op een leerling om alle eerdere antwoorden voor dat doel te zien.</p>
          </div>
                    ...
                    {latestRefs.map(({ userId, latest, count, all }) => {
                      ...
                      return (
                        <button 
                          ...
                          className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex flex-col h-full text-left transition-all hover:shadow-md hover:border-blue-200 group relative overflow-hidden"
                        >
                          ...
                          {latest.photoBase64 && (
                            <div className="mb-4 rounded-xl overflow-hidden border border-slate-100 aspect-video relative">
                              <img 
                          ...
                          <p className="text-slate-600 text-sm italic line-clamp-3 leading-relaxed relative z-10">
                            "{latest.content}"
                          </p>
      ...
      {activeTab === 'STUDENTS' && (
        <div className="animate-fadeIn">
          ...
          <form onSubmit={handleUserSubmit} className="flex gap-2 mb-8">
            <input
              type="text"
              value={newUserName}
              onChange={(e) => setNewUserName(e.target.value)}
              placeholder="Naam van de leerling..."
              className="flex-1 p-3 rounded-xl border-2 border-slate-200 outline-none focus:border-blue-400"
            />
            <button
              type="submit"
              disabled={!newUserName.trim()}
              className="bg-emerald-500 text-white font-bold py-2 px-6 rounded-xl shadow hover:bg-emerald-600 disabled:opacity-50 transition-colors"
            >
              Voeg toe
            </button>
          </form>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {users.map((user) => (
              <div key={user.id} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between group">
          ...
          {users.length === 0 && (
            <p className="text-center text-slate-400 mt-12 py-8 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200">
      ...
      {isAddingGoal && (
        <div className="fixed inset-0 bg-slate-900/50 flex items-center justify-center p-4 z-50 animate-fadeIn">
          ...
          >
            <h2 className="text-2xl font-bold mb-6 text-slate-800">Voeg lesdoel toe</h2>
            <div className="mb-4">
            ...
            <div className="mb-6">
              ...
              <textarea
                ...
                className="w-full h-24 p-3 rounded-xl border-2 border-slate-200 outline-none focus:border-blue-400 resize-none"
              />
            </div>
            <div className="flex gap-3">
              <button type="button" onClick={() => setIsAddingGoal(false)} className="flex-1 bg-slate-100 text-slate-600 font-bold py-3 rounded-xl">Annuleren</button>
              <button type="submit" className="flex-1 bg-blue-600 text-white font-bold py-3 rounded-xl">Opslaan</button>
            </div>
          </form>
        </div>
      )}
